...
public class ManipulacijaAnketama extends JFrame {
	...
	public void initialize() {
		...
		frmManipulacijaAnketama.getContentPane().add(panel1,
				BorderLayout.CENTER);
		panel1.setLayout(null);

		JPanel panel = new JPanel();
		panel.setBorder(new TitledBorder(null,
				"Ozna\u010Dite akciju i manipuli\u0161ite anketama",
		...
		// Testni podaci
		Object[] naziviKolona = new Object[] { "Broj", "Naziv",
				"Ograni\u010Denje", "Status" };
		...
		btnObriiOznaenu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				// BaseDao b=
				// BaseDao.delete(tblAnkete.getSelectionModel().getSelectionMode()4

				Kviz selected = new Kviz();
				int selectedRow = tblAnkete.getSelectedRow();
				if (selectedRow < 0) {
					JOptionPane.showMessageDialog(null,
							"Morate oznaciti anketu!");
				} else {

					selected = lk.get(tblAnkete
					...
					selected = (Kviz) session.load(Kviz.class,
							selected.get_id());
		...
		final JButton btnModifikujOznaenu = new JButton(
				"Modifikuj ozna\u010Denu");
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (selectedRow < 0) {
					...
					if (!kviz.is_aktivan() && !kviz.is_arhiviran()) {
						...
						ModifikacijaAnkete forma = new ModifikacijaAnkete(kviz,
								(JFrame) SwingUtilities.getRoot(component));
						frmManipulacijaAnketama.dispose();
						forma.setVisible(true);

					} else {
						JOptionPane.showMessageDialog(null,
								"Mozete modifikovati samo otvorenu anketu!");
					}
				}
		...
		frmManipulacijaAnketama.getContentPane().add(btnNewButton,
				BorderLayout.SOUTH);
...

		{
			...
			if (kviz.is_aktivan()) {
				model.addRow(new Object[] { kviz.get_id(), kviz.get_naziv(),
						kviz.get_vremenskoOgranicenje(), "aktivan" });

			} else if (kviz.is_arhiviran()) {
				model.addRow(new Object[] { kviz.get_id(), kviz.get_naziv(),
						kviz.get_vremenskoOgranicenje(), "arhiviran" });

			} else {
				model.addRow(new Object[] { kviz.get_id(), kviz.get_naziv(),
						kviz.get_vremenskoOgranicenje(), "otvoren" });
